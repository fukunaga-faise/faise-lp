...
export default function Entertainment() {
  ...
  return (
    <section className="relative bg-black py-16 md:py-36 px-6 overflow-hidden">
      <div
      ...
      <div className="relative max-w-5xl mx-auto" ref={ref}>
        ...
        {/* Stats row - horizontal on mobile too */}
        <motion.div
        ...
        >
          {stats.map((s, i) => (
            <div key={i} className="border border-white/[0.07] bg-white/[0.02] p-4 text-center rounded-sm">
              <p className="font-inter text-[clamp(20px,4vw,32px)] font-black text-gradient mb-2 leading-none">{s.value}</p>
              <p className="text-[10px] text-white/35 leading-[1.6] whitespace-pre-line">{s.label}</p>
            </div>
          ))}
        </motion.div>
        ...
        >
          <p className="text-[clamp(13px,1.7vw,16px)] text-white/55 leading-[2]">
            代表は<span className="text-white font-semibold">株式会社刀</span>で新規テーマパークのマーケに従事。
            集客戦略・広告開発・イベント企画を担当し、
            <span className="text-white font-semibold">施設内満足度No.1アトラクション</span>の開発を主導。
            テーマパークが研究してきた「人を熱狂させる設計」を店舗型ビジネスへ転用しています。
          </p>

          <blockquote className="border-l-2 border-blue-600 pl-6">
            <p className="text-[clamp(15px,2vw,20px)] font-bold leading-[1.8] text-white/80">
              人がわざわざ足を運ぶ理由は、&ldquo;正しさ&rdquo;ではなく&ldquo;
              <span className="text-white">体験したいという感情</span>&rdquo;にある。
            </p>
          </blockquote>
        </motion.div>
      </div>
